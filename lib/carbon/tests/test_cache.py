...
class MetricCacheShutdownTest(TestCase):
  """Regression tests ensuring cache drain during shutdown does not
  trigger flow control callbacks that would resume receiving."""

  def setUp(self):
    settings = {
      'MAX_CACHE_SIZE': 5,
      'CACHE_SIZE_HARD_MAX': 10,
      'CACHE_SIZE_LOW_WATERMARK': 5,
    }
    self._settings_patch = patch.dict('carbon.conf.settings', settings)
    self._settings_patch.start()
    self._orig_shutting_down = state.shuttingDown
    self._orig_cache_too_full = state.cacheTooFull
  ...
  def tearDown(self):
    ...
    state.cacheTooFull = self._orig_cache_too_full

  def test_pop_suppresses_cache_space_available_during_shutdown(self):
    """Cache drain below watermark with shuttingDown=True must not fire
    cacheSpaceAvailable."""
    # Fill cache above the low watermark (5).
    for i in range(8):
      self.metric_cache.store('metric.%d' % i, (123456 + i, float(i)))
    self.assertTrue(self.metric_cache.size >= 5)

    state.cacheTooFull = True
    state.shuttingDown = True

    with patch('carbon.cache.events') as events_mock:
      # Drain until below watermark.
      while self.metric_cache.size > 2:
        metric, _ = self.metric_cache.drain_metric()
        if metric is None:
          break
      events_mock.cacheSpaceAvailable.assert_not_called()

  def test_pop_fires_cache_space_available_normally(self):
    """Cache drain below watermark with shuttingDown=False must fire
    cacheSpaceAvailable (regression guard)."""
    for i in range(8):
      self.metric_cache.store('metric.%d' % i, (123456 + i, float(i)))
